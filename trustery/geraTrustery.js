...
while (nameList[k].slice(0,5) != nameList[1000].slice(0,5)) {
    //++++ Cria para cada Address Account um script de teste, baseado no "baseTrustery,js"
    shell.cat('baseTrustery.js').toEnd('/home/rogerio/AppsMC2/trustery/testDir/' +nameList[k].slice(0,5) +'.js');

    //++++ Aloca testAddress de para cada arquivo criado no intem anterior
    shell.exec(`sed -i "s/testAddress = ''/testAddress ='${nameList[k].slice(6,52)}'/" /home/rogerio/AppsMC2/trustery/testDir/` + nameList[k].slice(0,5) +'.js')

	//++++ Cria as chamadas dos arquivos ".js" através de arquivos ".py"
    shell.cat('/home/rogerio/AppsMC2/baseCall.py').to('/home/rogerio/AppsMC2/trustery/testDir/call'  +nameList[k].slice(0,5) +'.py');

    //++++ Aloca testAddress para cada chamada criada no intem anterior
	shell.exec(`sed -i "s/'node'/'node ${nameList[k].slice(0,5)}.js'/" /home/rogerio/AppsMC2/trustery/testDir/call` + nameList[k].slice(0,5) +'.py')
	k++
}
